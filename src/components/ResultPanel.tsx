...
export default function ResultPanel({ result, onRetry, onHome }: ResultPanelProps) {
  return (
    <div className="s2-inner">
      <p className="result-tag">운동 완료 · {result.exercise} {result.sets}세트</p>
      ...
      <div className="stat-cards">
        ...
        <div className="stat-card">
          <div className="sv">{result.durationMinutes}<span style={{ fontSize: 18, color: 'var(--muted)' }}>분</span></div>
          <div className="sl">운동 시간</div>
        </div>
        <div className="stat-card">
          <div className="sv" style={{ color: 'var(--accent2)' }}>{result.accuracy}%</div>
      ...
      <div className="feedback-box">
        ...
        {result.feedback.map((item, index) => (
          <div className="fb-item" key={`${item.status}-${index}`}>
            <span className="fb-ico">{item.status === 'ok' ? '✅' : '⚠️'}</span>
