...
class UIRenderer:
    ...
    def render(self, user_frame: np.ndarray, result: dict) -> np.ndarray:
        ...
        bad_joints: frozenset[int] = result.get("bad_joints", frozenset())  # type: ignore[assignment]
        user_panel = self._build_user_panel(user_resized, result["keypoints"], bad_joints)
    ...
    def _build_user_panel(
        ...
        bad_joints: frozenset[int],
    ) -> np.ndarray:
        panel = user_frame.copy()
        h, w = panel.shape[:2]
        self._draw_skeleton(panel, keypoints, h, w, bad_joints)
        return panel
    ...
    def _draw_skeleton(
        ...
        height: int,
        width: int,
        bad_joints: frozenset[int] = frozenset(),
    ) -> None:
        for start_idx, end_idx in SKELETON_EDGES:
            ...
            edge_is_bad = start_idx in bad_joints or end_idx in bad_joints
            if edge_is_bad:
                base_color = COLOR_BAD
            else:
                is_cross = (
        ...
        for idx in range(keypoints.shape[0]):
            ...
            if idx in bad_joints:
                color = _apply_alpha(COLOR_BAD, _conf_to_alpha(float(conf)))
                cv2.circle(frame, (int(x * width), int(y * height)), BAD_JOINT_RADIUS, color, -1)
            else:
