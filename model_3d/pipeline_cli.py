"""CLI runner and QA checks for the model_3d pipeline."""
...
def find_default_lifter_checkpoint() -> Optional[Path]:
    checkpoint_dir = package_root() / "artifacts" / "checkpoints"
    preferred_patterns = [
        "fitness_pose_lifter_latest_best.pt",
        "fitness_pose_lifter_latest.pt",
        "fitness_pose_lifter_overnight_best.pt",
        "fitness_pose_lifter_overnight.pt",
        "fitness_pose_lifter*_best.pt",
        "fitness_pose_lifter*.pt",
        "pose_lifter_latest.pt",
        "pose_lifter_smoke.pt",
    ]
    candidates: List[Path] = []
    seen: set[Path] = set()
    for pattern in preferred_patterns:
        for candidate in sorted(checkpoint_dir.glob(pattern), key=lambda path: path.stat().st_mtime, reverse=True):
...
def _looks_like_keypoints(value: Any) -> bool:
    if not isinstance(value, list) or len(value) != 17:
        return False
    return all(isinstance(row, list) and len(row) >= 2 for row in value)


def _print_summary(response: Dict[str, Any]) -> None:
    frame_id = response.get("frame_id", "unknown")
    status = response.get("status", "unknown")
    feedback = response.get("feedback", {}).get("label", "n/a")
    knee_angle = response.get("feedback", {}).get("knee_angle_deg", 0.0)
    model_ms = response.get("performance", {}).get("model_latency_ms", 0.0)
    artifact_count = len(response.get("diagnostics", {}).get("artifacts", {}))
    print(
        f"[pipeline] frame_id={frame_id} status={status} "
        f"feedback={feedback} knee={knee_angle:.2f} "
        f"model_ms={model_ms:.2f} artifacts={artifact_count}"
    )
...
def _payload_to_target_2d(payload: Any) -> np.ndarray:
    ...
    if array.shape != (17, 3):
        raise ValueError(f"Expected 17x3 keypoints for dummy fitter. Received {array.shape}.")
    x = array[:, 1] * 640.0
    y = array[:, 0] * 480.0
