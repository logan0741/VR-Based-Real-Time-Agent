...
## 1. 전체 시스템 흐름도 (Mermaid Diagram)
```mermaid
graph TD;
    A[Clients: 웹캠/영상/데이터셋] -->|"1. 실시간 2D/3D 키포인트 (17 joints)"| B[FastAPI Server];
    B -->|"2. PoseLifterNet (FastMLP) 추론"| C[SMPL-X 파라미터 66개 변환];
    C -->|"3. WebSocket Broadcast"| D[Unity AvatarController];
    D -->|"4. Rotation 보간 및 SMPLX.cs 렌더링"| E[Real-Time 3D Avatar];
```
...
## 4. 최종 아키텍처 다이어그램 (Mermaid)

최적화된 최종 실시간 아바타 솔루션 아키텍처는 아래와 같이 동작합니다.
...
graph TD
    subgraph Data_Sources [Data Sources]
        WC["Webcam Client"]
        VC["Video MP4 Client"]
        DS["pose_3d PKL Dataset"]
    end

    subgraph FastAPI_Backend [FastAPI Application Server]
        direction TB
        WS["WebSocket Endpoint: /ws/pose"]
        MLP["PoseLifterNet @torch.no_grad"]
        WS -->|"17 joints"| MLP
        MLP -->|"Fast Inference < 0.05ms"| Params["global_orient: 3, body_pose: 63"]
    end

    subgraph Unity_Frontend [Unity VR/3D Engine]
        direction TB
        UC["AvatarController.cs"]
        SM["SMPLX Model Rendering"]
        QUAT["SMPLX.QuatFromRodrigues"]
        
        UC -->|"Listen via ws://"| QUAT
        QUAT -->|"SetLocalJointRotation"| SM
    end

    Data_Sources -->|"JSON Payload (30FPS)"| WS
    Params -->|"Broadcast JSON"| UC
```
