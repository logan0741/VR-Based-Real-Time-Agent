...
| 모듈 | 질문 | 역할 |
|------|------|------|
| `FeedbackEngine` | **"지금 뭐가 틀렸나?"** | 프레임마다 분석 → 후보 dict 생성 |
| `FeedbackPolicy` | **"지금 보여줄 건가?"** | `target_fps × 2` 프레임(2초)마다 심각도 평균 기반으로 메시지 교체 |
...
result = {
    ...,
    "feedback": feedback_message,   # UIRenderer에 전달
}
...
FeedbackEngine         ← config를 읽어 분석 (로직 범용, 종목 무관)
        ↓
FeedbackPolicy         ← 표시 시점 제어 (target_fps×2 프레임 심각도 평균)
        ↓
UIRenderer             ← 메시지 화면 출력
```
...
config에 정의된 각 부위에 대해 **DTW 거리**와 **관절 위치 오차** 중 큰 값을 심각도로 삼는다.

```python
dtw_score  = max(joint_distances[start:end])          # DTW 거리 슬라이스 최댓값
pos_scores = [norm(user[j, :2] - expert[j, :2]) for j in joints]  # 관절별 위치 오차
score      = max(dtw_score, *pos_scores)
```

> DTW 거리는 시퀀스 전체 흐름의 오차, 위치 오차는 현재 프레임의 관절 좌표 차이.
> 둘 중 하나라도 크면 해당 부위가 문제라고 판단한다.

#### squat/side 현재 정의된 부위

| 부위 | DTW 슬라이스 | 위치 오차 관절 | threshold |
|------|-------------|---------------|-----------|
| `torso` | `[0:2]` (좌·우 어깨) | LEFT_SHOULDER, RIGHT_SHOULDER | 0.10 |
| `hip` | `[2:4]` (좌·우 골반) | LEFT_HIP, RIGHT_HIP | 0.10 |
| `knee` | `[4:6]` (좌·우 무릎) | LEFT_KNEE, RIGHT_KNEE | 0.12 |
| `ankle` | `[6:8]` (좌·우 발목) | LEFT_ANKLE, RIGHT_ANKLE | 0.10 |
...
### 변경 이력: 6프레임 → 48프레임 (심각도 평균)

기존에는 단순히 6프레임마다 최신 메시지로 교체했다.
24fps 기준 0.25초로, 사용자가 읽기 전에 메시지가 바뀌는 수준이었다.

현재는 **`target_fps × 2` 프레임(2초)마다** 교체하되,
그 사이에 쌓인 후보들 중 **severity 평균이 가장 높은 부위**의 메시지를 표시한다.
`update_interval`은 `FeedbackPolicy(update_interval=cfg["target_fps"] * 2)`로 종목별 target_fps에 따라 결정된다 (squat: 48, hammer_curl·pullup: 60).

### 진입점: `update()`

```python
update(frame_idx: int, candidate: dict[str, object] | None) -> str
```

```
실제 부위(pending·ok 제외) 후보 → severity를 _severity_buffer에 누적
                                  마지막 candidate를 _last_candidate_per_part에 저장

frame_idx % update_interval != 0  →  기존 메시지 유지

frame_idx % update_interval == 0  →  severity_buffer가 있으면 평균 최대 부위의 마지막 message로 교체
                                      severity_buffer가 비어 있고 candidate가 있으면 candidate message로 교체
                                   →  버퍼 초기화
```

### 왜 severity 평균인가

- 현재 프레임 한 장만 보면 일시적 노이즈(신뢰도 저하, 자세 전환 중)에 취약하다.
- `update_interval` 프레임 동안 지속적으로 심각도가 높았던 부위가 진짜 교정이 필요한 부위다.
- 평균 기반이므로 순간적으로 심각도가 튄 부위는 자연히 걸러진다.

### 초기 상태

인스턴스 생성 직후 `_active_message = "측정 중입니다."` 로 초기화되어 있어서,
DTW가 계산되기 전 구간에도 화면에 빈 칸 없이 표시된다.
...
[DTW 계산 주기 (n >= n_frames and n % dtw_interval == 0)]
    FeedbackEngine.analyze()
        → candidate dict 반환 (body_part, state, severity, message)
    그 외 프레임 → candidate = None

[매 프레임: FeedbackPolicy.update(frame_idx, candidate)]
    실제 부위면 severity 누적
    frame_idx % update_interval == 0 이면
        → update_interval 구간의 severity 평균 최대 부위 선택
        → _active_message 교체, 버퍼 초기화
    항상
        → _active_message 반환

[UIRenderer]
    → 중앙 패널에 _active_message 1개 표시
```
