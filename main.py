...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--preview",
        action="store_true",
        help="Preview all videos in assets/converted_videos and exit.",
    )
...
def preview_converted_videos() -> int:
    """converted_videos 폴더의 영상을 순서대로 재생한다."""
    folder = Path("assets/converted_videos")
    videos = sorted(folder.glob("*.mp4"))
    if not videos:
        print("[Preview] assets/converted_videos 에 영상이 없습니다.")
        return 1
    ...
    for path in videos:
        ...
        print(f"[Preview] {path.name}  (q: 다음, ESC: 종료)")
...
def main() -> int:
    ...
    if args.preview:
        return preview_converted_videos()
    ...
    if feedback_enabled:
        try:
            ...
            feedback_policy = FeedbackPolicy(hold_frames=cfg["target_fps"])
    ...
    feedback_bad_joints: frozenset[int] = frozenset()
    ...
    try:
        while True:
            ...
            result = {
                ...
                "bad_joints": feedback_bad_joints,
                "fps": 0.0,
            }
