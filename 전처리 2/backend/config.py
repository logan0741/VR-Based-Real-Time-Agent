...
EXERCISES: dict[str, dict] = {
    "squat": {
        ...
        # 피드백 엔진에 전달할 카메라 뷰 타입: "side" | "front"
        "view": "side",
    },
    "hammer_curl": {
        "video_path": "assets/expert_videos/hammer_curl.mp4",
        "normalizer_type": "side_left",
        "keypoints_used": [
    ...
    "pullup": {
        # 전문가 영상 파일 경로
        "video_path": "assets/expert_videos/pullup.mp4",

        # 정규화 기준 방향: "front"(정면)
        "normalizer_type": "front",

        # DTW 비교에 사용할 관절 인덱스 목록 (정면 풀업 — 상체 중심)
        "keypoints_used": [
            LEFT_SHOULDER, RIGHT_SHOULDER,
            LEFT_ELBOW, RIGHT_ELBOW,
        ],

        # 관절별 DTW 거리 가중치 (keypoints_used 순서와 1:1 대응)
        ...
        # 정규화 기준값 평활화에 사용할 프레임 수
        "norm_buffer_size": 7,

        # 1회 감지용 기울기 계산 윈도우 프레임 수
        "rep_slope_window": 20,

        # 유효 rep 최소 프레임 수 (target_fps × 1.5초 — 풀업은 스쿼트보다 동작이 느림)
        "min_rep_frames": 45,

        # DTW 계산 주기
        "dtw_interval": 3,

        # 피드백 엔진에 전달할 카메라 뷰 타입
        "view": "front",
    },
}
