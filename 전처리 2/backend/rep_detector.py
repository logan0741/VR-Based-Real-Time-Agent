...
SUPPORTED_DETECTOR_TYPES: frozenset[str] = frozenset({"squat", "pullup", "hammer_curl"})
...
class RepDetector:
    ...
    def _extract_signal(self, norm_frame: np.ndarray) -> float:
        """rep_detector_type과 normalizer_type에 따라 단일 프레임에서 rep 감지 신호를 추출한다. norm_frame shape=(17,3), dtype=float32."""
        if self._type == "pullup":
            wrist_y = float((norm_frame[LEFT_WRIST, 0] + norm_frame[RIGHT_WRIST, 0]) / 2.0)
            return float(norm_frame[NOSE, 0]) - wrist_y
        if self._type == "hammer_curl":
            if self._normalizer_type == "side_left":
                return float(norm_frame[LEFT_WRIST, 0]) - float(norm_frame[LEFT_SHOULDER, 0])
            return float(norm_frame[RIGHT_WRIST, 0]) - float(norm_frame[RIGHT_SHOULDER, 0])
        if self._normalizer_type == "side_left":
